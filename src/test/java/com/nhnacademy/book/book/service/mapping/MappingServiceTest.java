...
class MappingServiceTest {
    ...
    void processBookData_Success() {
        ...
        // Then
        assertTrue(result);
        verify(bookRepository, times(2)).save(any(Book.class)); // 두 번 저장되는 부분 확인
        verify(imageService, times(1)).saveBookImage(any(Book.class), eq("test_cover.jpg"));
        verify(categoryService, times(1)).saveBookCategory(any(Book.class), eq("Category > Subcategory"));
