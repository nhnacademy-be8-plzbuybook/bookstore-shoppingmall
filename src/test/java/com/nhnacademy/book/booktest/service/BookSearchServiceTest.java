...
class BookSearchServiceTest {
    ...
    @Test
    public void testSearchBooksByKeyword_NoBooksFound() {
        String keyword = "nonexistent";
        when(bookInfoRepository.searchBooksByKeyword(keyword)).thenReturn(Arrays.asList());

        BookNotFoundException exception = assertThrows(BookNotFoundException.class, () -> {
            bookSearchService.searchBooksByKeyword2(keyword, pageable);
        });
        assertEquals("No books found for keyword: nonexistent", exception.getMessage());
    }

    @Test
    public void testSearchBooksByKeyword_OutOfBoundsPage() {
        String keyword = "book";
        List<BookInfoDocument> books = List.of(
                new BookInfoDocument(1L, "Book 1", "Publisher A", "Category X", "Author A", new BigDecimal("10000"), new BigDecimal("12000"), "url1", 101L)
        );
        when(bookInfoRepository.searchBooksByKeyword(keyword)).thenReturn(books);

        IllegalArgumentException exception = assertThrows(IllegalArgumentException.class, () -> {
            bookSearchService.searchBooksByKeyword2(keyword, PageRequest.of(1, 2)); // 페이지 1로 요청 (범위 초과)
        });
    }
}
